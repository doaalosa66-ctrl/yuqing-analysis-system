...
**信源溯源要求（重要）：**
- `sources` 数组必须包含本段落引用的所有信息来源
- 每条 source 必须包含 `title`（媒体/文章标题）和 `url`（原始链接）
- 如果搜索结果中有 `published_date`，也请填入
- 在 `paragraph_latest_state` 正文中，每处引用数据或关键结论后，用 `([来源名称](URL))` 格式内联标注信源
- 无法获取 URL 的内部数据，url 字段填写 "系统内部采集"
...
**信源溯源要求（重要）：**
- `sources` 数组必须包含本次反思补充的所有信息来源
- 每条 source 必须包含 `title`（媒体/文章标题）和 `url`（原始链接）
- 如果搜索结果中有 `published_date`，也请填入
- 在 `updated_paragraph_latest_state` 正文中，每处新增的引用数据或关键结论后，用 `([来源名称](URL))` 格式内联标注信源
- 无法获取 URL 的内部数据，url 字段填写 "系统内部采集"
...
### AI分析结果汇总
### 数据溯源与信源矩阵
必须以 Markdown 表格的形式输出，包含以下列：
| 信源权重 | 平台/媒体名称 | 核心观点/数据摘要 | 原始链接 (URL) | 发布时间 |
|----------|---------------|-------------------|----------------|----------|
| 高       | XX媒体        | 核心观点摘要      | [链接](URL)    | YYYY-MM-DD |

*注：请将所有段落中引用的有效外链全部整理至此表中，确保每一条决策都有迹可循。如原始数据无URL，请保留媒体名称并标注"系统内部采集"。*
```
