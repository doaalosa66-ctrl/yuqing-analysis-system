...
class MediaCrawlerDB:
    ...
    def search_hot_content(
        self,
        time_period: Literal['24h', 'week', 'month', 'year'] = 'month',
        limit: int = 50
    ) -> DBResponse:
        """
        ...
        Args:
            time_period (Literal['24h', 'week', 'month', 'year']): 时间范围，默认为 'month'(3个月)。
        ...
        now = datetime.now()
        start_time = now - timedelta(days={'24h': 1, 'week': 7, 'month': 90}.get(time_period, 90))
