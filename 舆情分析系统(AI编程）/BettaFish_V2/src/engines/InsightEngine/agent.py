...
class DeepSearchAgent:
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        if search_tool == "search_hot_content":
            # 强制使用配置的时间范围，防止LLM自行设置过长的时间
            time_period = self.config.DEFAULT_TIME_PERIOD
    ...
    def _generate_final_report(self) -> str:
        ...
        for paragraph in self.state.paragraphs:
            ...
            for search in paragraph.research.search_history:
                if search.url and search.url not in seen_urls:
                    ...
                    all_sources.append({
                        "title": search.title or "无标题",
                        "url": search.url,
                        "platform": self._extract_platform_from_url(search.url)
                    })
        ...
        # 添加参考文献列表
        if all_sources:
            references_section = self._generate_references_section(all_sources)
        ...
        logger.info(f"最终报告生成完成，包含 {len(all_sources)} 条参考文献")
        return final_report
    ...
    def _generate_references_section(self, sources: list) -> str:
        ...
        section = "\n\n---\n\n## 📚 参考文献\n\n"
        section += f"<p>本报告共引用 {len(sources)} 条来源，数据时间范围：最近3个月</p>\n\n"
        section += "<table style='width:100%; border-collapse:collapse; font-size:14px;'>\n"
        section += "<tr style='background:#f5f5f5;'>"
        section += "<th style='padding:10px; border:1px solid #ddd; text-align:left;'>平台</th>"
        section += "<th style='padding:10px; border:1px solid #ddd; text-align:left;'>标题</th>"
        section += "<th style='padding:10px; border:1px solid #ddd; text-align:left;'>链接</th>"
        section += "</tr>\n"

        for source in sources:
            platform = source.get("platform", "未知")
            title = source.get("title", "无标题")
            url = source.get("url", "#")
            # 转义HTML特殊字符
            title_escaped = title.replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
            section += "<tr>"
            section += f"<td style='padding:8px; border:1px solid #ddd;'>{platform}</td>"
            section += f"<td style='padding:8px; border:1px solid #ddd; max-width:300px; overflow:hidden; text-overflow:ellipsis;'>{title_escaped}</td>"
            section += f"<td style='padding:8px; border:1px solid #ddd;'><a href='{url}' target='_blank' style='color:#1a73e8; text-decoration:none;'>查看原文 →</a></td>"
            section += "</tr>\n"
