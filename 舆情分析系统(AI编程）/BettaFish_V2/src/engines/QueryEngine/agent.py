...
class DeepSearchAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        # 初始化搜索工具集（Tavily key 缺失时记录警告，不崩溃）
        if self.config.TAVILY_API_KEY:
            self.search_agency = TavilyNewsAgency(api_key=self.config.TAVILY_API_KEY)
        else:
            logger.warning("[QueryEngine] TAVILY_API_KEY 未配置，QueryEngine 将跳过网络新闻搜索")
        ...
        logger.info(f"搜索工具集: {'TavilyNewsAgency' if self.search_agency else '未配置（跳过网络搜索）'}")
    ...
    def execute_search_tool(self, tool_name: str, query: str, **kwargs) -> TavilyResponse:
        ...
        if self.search_agency is None:
            logger.warning(f"  ⚠️  search_agency 未初始化（TAVILY_API_KEY 未配置），跳过搜索")
