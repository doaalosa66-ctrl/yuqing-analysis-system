...
:root {{ /* 含义：亮色主题变量区域；设置：在本块内调整相关属性 */
  ...
  /* ===== IBM 风格专业配色方案 ===== */
  --ibm-blue: #0066a1;
  --ibm-light-blue: #e4edf4;
  --ibm-dark-blue: #003d6b;
  --ibm-gray: #5a6872;
  --ibm-light-gray: #f5f7fa;
  --ibm-border: #d0d7dc;
...
h2 {{ font-size: 1.5rem; font-weight: 600; border-bottom-width: 2px; }}
h3 {{ font-size: 1.25rem; font-weight: 600; border-bottom-width: 1px; }}
...
table th, table td {{ /* 含义：表格单元格；设置：在本块内调整相关属性 */
  padding: 8px 13px; /* 含义：内边距，控制内容与容器边缘的距离；设置：按需调整数值/颜色/变量 */
  border: 1px solid var(--ibm-border); /* 含义：边框样式；设置：按需调整数值/颜色/变量 */
}} /* 结束 table th, table td */
table th {{ /* 含义：table th 样式区域；设置：在本块内调整相关属性 */
  background: var(--ibm-light-blue); /* 含义：背景色或渐变效果；设置：按需调整数值/颜色/变量 */
  color: var(--ibm-blue);
  font-weight: bold;
  font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;
}}
