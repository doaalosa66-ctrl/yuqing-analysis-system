...
OPS_ENV: Path = PROJECT_ROOT / "ops" / "config" / ".env"
ENV_FILE: str = str(OPS_ENV if OPS_ENV.exists() else (PROJECT_ROOT / ".env"))
...
class Settings(BaseSettings):
    ...
    OUTPUT_DIR: str = Field(str(PROJECT_ROOT / "outputs" / "final_exports"), description="主输出目录")
    CHAPTER_OUTPUT_DIR: str = Field(
        str(PROJECT_ROOT / "outputs" / "final_exports" / "chapters"), description="章节JSON缓存目录"
    )
    DOCUMENT_IR_OUTPUT_DIR: str = Field(
        str(PROJECT_ROOT / "outputs" / "final_exports" / "ir"), description="整本IR/Manifest输出目录"
    )
    CHAPTER_JSON_MAX_ATTEMPTS: int = Field(
        2, description="章节JSON解析失败时的最大尝试次数"
    )
    TEMPLATE_DIR: str = Field(str(PROJECT_ROOT / "src" / "engines" / "ReportEngine" / "report_template"), description="多模板目录")
    ...
    LOG_FILE: str = Field(str(PROJECT_ROOT / "ops" / "logs" / "report.log"), description="日志输出文件")
    ...
    JSON_ERROR_LOG_DIR: str = Field(
        str(PROJECT_ROOT / "ops" / "logs" / "json_repair_failures"), description="无法修复的JSON块落盘目录"
    )
