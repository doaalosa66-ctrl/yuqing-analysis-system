...
class ReportAgent:
    ...
    def _build_generation_context(
        ...
        return {
            ...
            "data_bundles": [],
            "max_tokens": min(self.config.MAX_CONTENT_LENGTH, 8192),
    ...
    _MAP_SYSTEM_PROMPT = (
        ...
        "【重要】必须严格以 JSON 格式输出，格式为：{\"facts\": [\"要点1\", \"要点2\", ...]}，不要包含任何其他说明文字。"
    )
    _REDUCE_SYSTEM_PROMPT = (
        "你是一位专业的报告整合助手。以下是同一主题的多个分块摘要，"
        "请将它们整合为一份连贯、完整的综合摘要，保留所有重要信息，"
        "消除重复，确保逻辑流畅。直接输出整合后的摘要，不要加任何前缀。"
        "【重要】必须严格以 JSON 格式输出，格式为：{\"summary\": \"整合后的完整摘要文本\"}，不要包含任何其他说明文字。"
    )
