...
def get_result(task_id):
    ...
    project_root = project_root.parent
    reports_dir = project_root / 'outputs' / 'raw_md'
    logger.info(f"[get_result] project_root={project_root}, reports_dir={reports_dir}")
    ...
    if reports_dir.exists() and reports_dir.is_dir():
        try:
            ...
            logger.error(f"[get_result] 读取异常: {e}\n{traceback.format_exc()}")
    ...
    try:
        ...
        if not pango_available:
            return jsonify({
                ...
                'help_url': 'https://github.com/doaalosa66-ctrl/Mina#2-安装-pdf-导出所需系统依赖可选',
