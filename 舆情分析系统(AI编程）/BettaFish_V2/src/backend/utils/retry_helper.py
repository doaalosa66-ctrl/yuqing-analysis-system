...
# 导入 TokenBudgetExceeded，重试时必须跳过此异常
try:
    from utils.token_tracker import TokenBudgetExceeded as _TokenBudgetExceeded
except ImportError:
    try:
        from token_tracker import TokenBudgetExceeded as _TokenBudgetExceeded
    except ImportError:
        _TokenBudgetExceeded = None
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(config.max_retries + 1):  # +1 因为第一次不算重试
                try:
                    ...
                    # ===== Token 预算耗尽：立刻抛出，禁止重试 =====
                    if _TokenBudgetExceeded and isinstance(e, _TokenBudgetExceeded):
                        logger.warning(f"函数 {func.__name__} 触发 Token 预算熔断，立刻抛出，禁止重试")
                        raise
                    # ===== 熔断结束 =====
