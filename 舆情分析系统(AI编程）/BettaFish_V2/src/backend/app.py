...
_PROJECT_ROOT = src_dir.parent
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))
# V2 架构：引擎包在 src/engines，工具包在 src/backend
# retry_helper 在 src/backend/utils/，需要能被引擎子目录（llms/tools）找到
_SRC_ROOT = _PROJECT_ROOT / 'src'
_BACKEND_DIR = _SRC_ROOT / 'backend'
for _p in [str(_SRC_ROOT), str(_BACKEND_DIR)]:
...
try:
    ...
    MINDSPIDER_AVAILABLE = True
except ImportError:
    MINDSPIDER_AVAILABLE = False
    logger.warning("MindSpider 模块未找到，数据库初始化功能不可用")

try:
    from engines.ReportEngine.flask_interface import report_bp, initialize_report_engine
...
def initialize_system_components():
    """启动所有依赖组件（ReportEngine）。ForumEngine 和 MindSpider 已废弃，系统改用纯 Python runner。"""
    ...
    logger.info("[系统启动] MindSpider 已废弃，跳过数据库初始化")
    _emit_start_progress('db', 'running', '数据库初始化跳过（MindSpider已废弃）')
    logs.append("MindSpider 已废弃，数据库初始化跳过")

    # ---- ForumEngine（已废弃，注释所有相关代码）----
    # try:
    #     stop_forum_engine()
    # except Exception:
    #     pass
    ...
    processes['forum']['status'] = 'stopped'
    logger.info("[系统启动] ForumEngine 已废弃，跳过启动")
...
# 初始化ForumEngine的forum.log文件（已废弃）
# def init_forum_log():
#     """初始化forum.log文件"""
#     try:
...
# init_forum_log()  # ForumEngine 已废弃

# 启动ForumEngine智能监控（已废弃）
...
def start_app(app_name):
    ...
    if app_name == 'forum':
        # ForumEngine 已废弃
        return jsonify({'success': False, 'message': 'ForumEngine 已废弃，请使用搜索功能'})
...
def stop_app(app_name):
    ...
    if app_name == 'forum':
        # ForumEngine 已废弃
        return jsonify({'success': False, 'message': 'ForumEngine 已废弃'})
...
def search():
    ...
    def _do_search():
        ...
        import time as _time
        from src.backend.runner import run_pipeline
        _t0 = _time.time()
...
def get_report_result(task_id):
    ...
    # 1. 尝试从内存获取
    task = _search_tasks.get(task_id)
    if task and task.get('status') in ['done', 'completed']:
        result = task.get('result', {})
        if 'html_content' in result:
            return result['html_content'], 200
        report_file = result.get('report_filepath') or result.get('report_file')
        if report_file:
            p = Path(report_file)
            if p.exists():
    ...
    if reports_dir.exists() and reports_dir.is_dir():
        for file_path in reports_dir.rglob('*.md'):
            if task_id in file_path.name:
                ...
                return markdown.markdown(md_content, extensions=['tables', 'fenced_code']), 200

    # 3. 扫描 HTML 文件
    html_reports_dir = _PROJECT_ROOT / 'outputs' / 'final_exports'
    if html_reports_dir.exists() and html_reports_dir.is_dir():
        for file_path in html_reports_dir.rglob('*.html'):
            if task_id in file_path.name:
                html_content = file_path.read_text(encoding='utf-8')
                return html_content, 200

    return '<div style="text-align:center;padding:50px;"><h2>404 报告未找到</h2><p>报告生成中或已被清理，请稍后重试。</p></div>', 404


@app.route('/api/report/history', methods=['GET'])
def get_report_history():
    """扫描报告目录获取列表（同名去重，保留最新）"""
    import os
    from datetime import datetime
    ...
    try:
        for fname in os.listdir(reports_dir):
            if fname.endswith('.md'):
                if fname.startswith('deep_search_report_'):
                    fname_for_parse = fname.replace('deep_search_report_', '', 1)
                elif fname.startswith('report_'):
                    fname_for_parse = fname.replace('report_', '', 1)
                else:
                    continue

                parts = fname_for_parse.replace('.md', '').rsplit('_', 2)
                if len(parts) >= 3:
                    title = parts[0]
                    date_str = parts[1]
                    time_str = parts[2]
