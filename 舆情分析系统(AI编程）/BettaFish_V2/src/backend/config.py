...
# 计算 .env 优先级：优先当前工作目录，其次 ops/config/.env，最后项目根目录
PROJECT_ROOT: Path = Path(__file__).resolve().parent.parent.parent  # BettaFish_V2/
CWD_ENV: Path = Path.cwd() / ".env"
OPS_ENV: Path = PROJECT_ROOT / "ops" / "config" / ".env"
ENV_FILE: str = str(
    CWD_ENV if CWD_ENV.exists()
    else OPS_ENV if OPS_ENV.exists()
...
def reload_settings() -> Settings:
    ...
    ops_env = PROJECT_ROOT / "ops" / "config" / ".env"
    env_file = str(
        cwd_env if cwd_env.exists()
        else ops_env if ops_env.exists()
        else (PROJECT_ROOT / ".env")
